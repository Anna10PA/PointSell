    ...
    return (
        <div className={` px-5 py-4 w-full duration-100 rounded-xl ${show === order ? 'bg-[#f67f20] hover:bg-orange-400 text-white' : 'bg-[#F5F5F5] hover:bg-gray-200'}`} onClick={()=> {
            ...
            <div className='flex items-center justify-between'>
                ...
                <h2 className={`${(userOrders?.includes(order) || user === 'futureana735@gmail.com') && show !== order ? 'text-[green] font-bold' : show === order && ( userOrders?.includes(order) || user === 'futureana735@gmail.com')? 'font-bold text-white' : userOrders?.includes(order) || user === 'futureana735@gmail.com'  ? 'bg-[#F5F5F5] hover:bg-gray-200' : 'hidden'}`}>${Number(pay || 0).toFixed(2)}</h2>
            </div>
            <div>
