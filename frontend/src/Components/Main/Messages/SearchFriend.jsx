        ...
        <main className="w-full flex items-start gap-5">
            <section className="w-full h-[95vh] flex flex-col px-10 py-5 gap-5">
                <header className="flex items-center justify-between w-full gap-5 min-h-[10vh] ">
                    <h1 className="text-3xl font-bold">Search Friend</h1>
                    <div className='w-[40%] max-[500px] min-[100px] relative  '>
                ...
                <section className="w-full h-full">
                    {
                        foundUsers?.length > 0 ? foundUsers?.map((item, index) => {
                            return <User 
                            key={index}
                            name={item.name}
                            image={item.profileUrl}
                            email={item.email}
                            sender={curentUser?.friends}
                            />
                        }) :
                            <div className="w-full h-full flex items-center justify-center flex-col gap-3">
                                ...
                                <div className="flex flex-col items-center gap-2">
                                    <h1 className="font-bold text-3xl">Find a Friend</h1>
                                    <p className="text-gray-400">Enter your friend's email and start chatting!</p>
                                </div>
                            </div>
                    }
                </section>
