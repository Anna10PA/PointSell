...
function SearchFriend() {
    ...
    return (
        <main className="w-full flex items-start gap-5">
            <section className="w-full h-[95vh] flex flex-col px-10 py-5 gap-5 max-sm:px-3 max-sm:py-2">
                <header className="flex items-center justify-between w-full gap-5 min-h-[10vh] relative">
                    <h1 className="text-3xl font-bold max-sm:text-[25px]">{location_name === '/main/requests' ? 'Requests' : 'Search Friend'}</h1>
                    <form className={`duration-300 transition-all relative bg-white ${location_name === '/main/requests' ? 'hidden' : 'flex'} ${openSearch
                            ? 'max-sm:absolute max-sm:left-0 max-sm:w-full max-sm:z-30'
                            : 'w-[40%] max-sm:w-10'}`}>
                        ...
                        <i className="fa-solid fa-magnifying-glass absolute right-5 top-1.5 text-[#bbb] h-7 flex items-center justify-center max-sm:right-3 rounded-[50%] bg-white pl-3 pt-2"  onClick={() => {
                                if (window.innerWidth < 648) {
                                    setOpenSearch(!openSearch)
                                }
                            }}></i>
                    </form>
                    <div className='w-12 h-12 rounded-[50%] bg-[#f67f20] flex items-center justify-center text-white cursor-pointer duration-100 hover:bg-orange-400 max-sm:w-10 max-sm:h-10 max-sm:text-[14px]' onClick={() => {
