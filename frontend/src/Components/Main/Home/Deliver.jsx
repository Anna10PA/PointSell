...
function Deliver() {
    ...
    let { register, handleSubmit, reset, formState: { errors }, trigger } = useForm({
        mode: "onBlur"
    })
            ...
            <main className="w-full h-full flex flex-col px-10 py-5 gap-5">
                <header className="flex items-center justify-between w-full gap-5 min-h-[10vh] relative">
                    <h1 className="text-3xl font-bold">
                        Order #{curentUser ?
                            curentUser.curent_cart.order : 'Loading . . . '}
                    </h1>
                    <Link to='/order_type'>
                ...
                <form className="flex flex-col gap-3" onSubmit={handleSubmit((data) => {
                    ...
                    <ProductAndChack curentUser={curentUser} tax={deliverInfo.distance ? (deliverInfo.distance * 2 + 1.5) : 0} />
                </form>
            </main>
        </>
    )
